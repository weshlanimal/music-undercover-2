...
- **Chat textuel pendant l'écoute et la discussion** : permet de jouer sans discussion vocale à côté. Panneau toujours visible (pas besoin de cliquer pour l'ouvrir) ; l'historique de la manche est visible par tout le monde (y compris un joueur qui vient de rejoindre en retard) et repart à zéro à chaque nouvelle manche. Fermé pendant le vote et les autres phases.
- **Missions aléatoires et privées** : à chaque manche, certains joueurs (au moins un, garanti) reçoivent en plus de leur thème une contrainte de genre musical ("Rap français", "Générique d'anime", "Musique sans parole"…) — révélée en même temps que le thème, connue du seul joueur concerné. Personne d'autre, y compris les autres joueurs et l'état de la partie, ne sait qui a une mission ni laquelle. Pas vérifiée automatiquement (pas de métadonnée de genre fiable sur YouTube) — une règle de bonne foi entre joueurs, comme au Jackbox. La liste vit dans `data/missions.csv`, éditable exactement comme les thèmes.
...
- **Missions privées, transmises par le même canal que le thème.** Assignées une fois par manche (pas par tour) en même temps que rôle et thème, elles voyagent dans le secret privé du joueur (`PrivatePlayerSecret.mission`) — jamais dans l'état public de la room. Ça les fait bénéficier gratuitement du mécanisme d'auto-réparation déjà en place pour le thème (voir plus bas) : un message initial perdu se corrige tout seul, mission comprise. La garantie "au moins une mission par manche" (`assignMissionsForRound`) tire d'abord indépendamment pour chaque joueur, puis force une mission sur un joueur choisi au hasard SEULEMENT si personne n'en a reçu — pour ne pas transformer la garantie en quasi-certitude que tout le monde en ait une.
- **Missions non vérifiées automatiquement — une règle de bonne foi, pas une contrainte technique.** Il n'existe aucune métadonnée de genre musical fiable sur un lien YouTube (l'API oEmbed publique ne renvoie que titre/auteur/miniature). Plutôt que de construire une détection de genre peu fiable (et probablement frustrante quand elle se trompe), la mission est laissée à l'honnêteté du joueur concerné — comme la plupart des contraintes dans les party games façon Jackbox.
- **Chat textuel toujours visible, panneau intégré au flux plutôt que bulle à ouvrir.** Positionner un panneau flottant à ouvrir semblait pratique, mais ça revient à cacher des messages qui viennent d'arriver — à l'inverse de l'objectif (remplacer une discussion vocale, où on n'a pas besoin de "cliquer pour écouter"). Le panneau est donc rendu directement dans le flux normal de l'écran (pas de position fixe, pas d'état ouvert/fermé), toujours visible pendant l'écoute et la discussion.
- **Chat textuel conservé dans l'état de la room, pas éphémère comme les réactions emoji.** Les réactions emoji sont un pur événement transitoire (`broadcastToRoom`, jamais stocké) parce que leur seule fonction est l'animation à l'instant T. Les messages de chat, eux, sont ajoutés à `room.chatMessages` et transitent par `broadcastState` : un joueur qui rejoint en retard voit l'historique complet de la manche, pas seulement les messages arrivés après son arrivée. Remis à zéro à chaque nouvelle manche pour ne pas accumuler indéfiniment.
...
- `npm test` — points individuels par rôle/survie, résolution à la pluralité (cible unique), résolution mock, extraction d'ID YouTube, intégrité de la base de thèmes ET de missions (pas de doublon, pas de vide, plusieurs catégories) lues en direct depuis leurs CSV, probabilité de tirage d'une mission par joueur (largement minoritaire, jamais nulle) et garantie qu'il y en ait toujours au moins une par manche.
- `node scripts/e2e-smoke-test.mjs` (serveur démarré en parallèle) — 11 scénarios indépendants via Socket.io :
  ...
  - **le chat textuel (ouvert seulement pendant l'écoute et la discussion, refusé en lobby/vote, diffusé à tout le monde) et la confidentialité des missions (aucune trace dans l'état public, chaque joueur reçoit uniquement la sienne, au moins un joueur en a une par manche)** ;
